...
export default class AppLogin extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            setpolelogin:false,
            setpolepass:false,
            errorlogin: 'Login cannot be empty',
    ...
    blurHandrel = event =>{   
        switch (event.target.name){
            case 'login': 
                this.setState(() => ({ setpolelogin: true })); 
                break
            case 'password':
                this.setState(() => ({ setpolepass: true }));
                break
        }
    }
    openModal = (event) => {
        ...
        axios.post("api/auth/login", { ss }).then((resp) => {console.log(resp.data);
                            ...
                            <div className="">
                                <form>
                                    <div className="contentlogin">
                                        {( this.state.errorlogin && this.state.polelogin ) && <div style={{color:"red",textAlign: 'center'}}>{this.state.errorlogin}</div>}
                                        <div className="inputtext">
                                            ...
                                            <input onBlur={e =>this.blurHandrel(e)} name="login"  autoFocus={true} placeholder="Enter login" value={this.state.login} onChange={this.handleInputChange} required/>
                                        </div>
                                        {( this.state.errorpass && this.state.polepass ) && <div style={{color:"red"}}>{this.state.errorpass}</div>}
                                        <div className="inputtext">
                                            ...
                                            <input onBlur={e =>this.blurHandrel(e)}  type="password" name="password" placeholder="Enter password" value={this.state.password} onChange={this.handleInputChange} required type="password"/>
                                        </div>
                                    </div>
                                </form>
